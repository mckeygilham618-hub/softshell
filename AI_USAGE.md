...
**功能设计。** 表情包文件夹机制、以文件名作为语义标签、AI 主动向用户索要素材、头像位留空由用户自填、不引导用户建立情感陪伴关系、多会话与群聊的微信语义（@点名发言、成员各用各的模型）、聊天记录的查找/置顶/导出、表情包作为独立消息渲染——这些设计出自作者。作者还会收集「被试」（运行在软壳里的 Claude 实例）的使用反馈，并决定是否采纳（如表情包文件夹改名、外观文件容错）。
