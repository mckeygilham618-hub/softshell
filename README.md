...
点📞弹出两种模式，按你当时是不是盯着屏幕来选。两种都是：点一下开麦，直接说；说完停一下自动发送，麦克风随即闭麦；一轮结束自动再开麦，你接着说。都没有单独的通话页面——你说的话和它的回复都以普通气泡实时出现在聊天窗里（思考链、工具调用照常显示）。再点📞关掉。

**🎙 语音输入**——只把你的话转成文字发出去，Claude 的回复**不朗读**，你用眼睛看。就是替你省下打字，其余和平常打字聊天一模一样：回复照常排版，该有表格、代码块、表情包就有，思考档位也照你在状态栏选的跑（开麦那一刻的设置，中途改要重新开麦才生效）。

**📞 对讲机**——你语音说，Claude 的回复**边生成边逐句念出来**，念完自动再开麦。适合你正在看别的软件、不盯着聊天窗的场景；窗口缩小、切去干别的都不影响轮转。**它念的时候你一开口，它就停下来听你的。**这个模式下它会用口语短句回话、思考档位压到 low，图的是秒回。
